...
def defined_translation(
    ...
    """
    This function simply create an xarray directly from a function that is applied to every chunk.
...
def _generate_lazy_data_array(
    ...
    """
    Create an Xarray DataArray from a function applied on every chunk
    """

    def only_coords(x):
        return func(coords=x.coords, **func_parameters)

    arr = empty_xarray(dims, coords, chunks, dtype)
    return arr.map_blocks(only_coords)
...
def _generate_lazy_dataset(
    ...
    return dataset.map_blocks(only_coords)
